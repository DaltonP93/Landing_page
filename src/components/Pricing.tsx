      ...
      <div className="max-w-6xl mx-auto px-6">
        <div ref={headerRef} className={`reveal ${headerVisible ? 'visible' : ''} text-center max-w-2xl mx-auto mb-12`}>
          ...
          <div className="mt-8 inline-flex items-center gap-1 p-1 rounded-lg border border-border/30 glass">
            <button
              className={`px-5 py-2 rounded-md text-xs font-medium transition-all duration-300 ${
                !annual
                  ? 'bg-gradient-to-r from-neon-purple to-neon-blue text-white shadow-[0_0_15px_rgba(124,58,237,0.3)]'
            ...
            <button
              className={`px-5 py-2 rounded-md text-xs font-medium transition-all duration-300 flex items-center gap-1.5 ${
                annual
                  ? 'bg-gradient-to-r from-neon-purple to-neon-blue text-white shadow-[0_0_15px_rgba(124,58,237,0.3)]'
                  : 'text-muted hover:text-white'
              }`}
...
function PricingCard({ product, annual, index }: { product: typeof products[0]; annual: boolean; index: number }) {
  ...
  const price = annual ? product.pricing.annual : product.pricing.monthly;
  ...
  return (
    ...
    >
      <div className={`relative rounded-xl p-6 transition-all duration-500 h-full flex flex-col neon-border ${
        isPopular
          ? 'glass-strong shadow-[0_0_40px_rgba(124,58,237,0.15)] border-neon-purple/30'
          : 'glass hover:shadow-[0_0_25px_rgba(0,212,255,0.06)]'
      }`}>
        {isPopular && (
          <div className="absolute -top-2.5 left-6 px-3 py-0.5 rounded-full text-[10px] font-bold bg-gradient-to-r from-neon-purple to-neon-blue text-white tracking-wide uppercase shadow-[0_0_12px_rgba(124,58,237,0.4)]">
            ⚡ Popular
          </div>
        )}
        ...
        <div className="mb-5">
          <span className="text-3xl font-bold bg-gradient-to-r from-white to-white/80 bg-clip-text text-transparent">{formatPYG(price)}</span>
        ...
        <a
          href="#demo"
          className={`block w-full text-center py-2.5 rounded-lg text-xs font-medium transition-all duration-300 ${
            isPopular
              ? 'bg-gradient-to-r from-neon-purple to-neon-blue text-white shadow-[0_0_20px_rgba(124,58,237,0.3)] hover:shadow-[0_0_30px_rgba(0,212,255,0.4)]'
              : 'border border-border/40 text-muted hover:text-neon-blue hover:border-neon-blue/30 hover:shadow-[0_0_15px_rgba(0,212,255,0.1)]'
          }`}
        >
